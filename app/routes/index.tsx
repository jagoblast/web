...
export default createRoute(async (c) => {
  ...
  const renderWidget = async (widget: any) => {
    ...
    if (widget.widget_type === 'icon_nav') {
      ...
      return (
        <section key={widget.id} className="w-full bg-white py-8 md:py-12 border-b border-gray-100">
    ...
    // 3. WIDGET: PROMO BANNER (Grid Kampanye)
            ...
            {promos.map((promo: any, idx: number) => (
              <a key={idx} href={promo.link} className="group relative block overflow-hidden bg-gray-100 rounded-sm">
                ...
                <div className="absolute inset-0 bg-black bg-opacity-20 group-hover:bg-opacity-30 transition-all flex flex-col items-center justify-center text-white p-6 text-center">
                  <h3 className="text-3xl md:text-4xl font-black uppercase tracking-widest mb-3 drop-shadow-md">{promo.title}</h3>
                  {promo.subtitle && <p className="text-sm font-medium mb-6 drop-shadow-md max-w-sm">{promo.subtitle}</p>}
                  <span className="bg-white text-black px-8 py-3 text-xs font-bold uppercase tracking-widest group-hover:bg-gray-200 transition-colors rounded-sm">
    ...
    // 4. WIDGET: MARKETPLACE PRODUCT GRID
    if (widget.widget_type === 'featured_products' || widget.widget_type === 'new_arrivals') {
      ...
      // PERBAIKAN: Melakukan JOIN ke tabel stores untuk mengambil nama Boutique Penjual
      ...
      return (
        <section key={widget.id} className="w-full bg-white py-12 md:py-16 px-4 md:px-8">
          <div className="max-w-7xl mx-auto">
            
            <div className="flex flex-col items-center mb-10">
              <h2 className="text-2xl md:text-3xl font-black text-gray-900 uppercase tracking-widest mb-3 text-center">{widget.title}</h2>
              {content.description && <p className="text-sm text-gray-500 text-center max-w-2xl mb-4">{content.description}</p>}
              <a href="/products" className="text-xs font-bold text-gray-900 border-b-2 border-black pb-1 hover:text-gray-500 transition-colors uppercase tracking-widest">
                {content.button_text || 'Lihat Semua Koleksi'}
              </a>
            </div>
            
            {/* GRID ALA ASOS: Minimalis, Gambar Panjang, Info Toko */}
                  ...
                  >
                    ...
                    <div className="w-full aspect-[3/4] bg-gray-100 relative overflow-hidden mb-3">
                      <img 
                        ...
                        loading="lazy"
                      />
                      {/* Badge Kondisi / Tag (Opsional) */}
                      <div className="absolute top-2 left-2 bg-white px-2 py-1 text-[9px] font-black uppercase tracking-widest text-black">
                        {product.brand}
                      </div>
                    </div>
                    
                    {/* Informasi Produk (Nama Boutique Paling Menonjol) */}
  ...
  return c.render(
    <div className="bg-white min-h-screen">
      {/* Inject CSS Global untuk menyembunyikan Scrollbar Slider */}
