...
export default createRoute(async (c) => {
  ...
  // Ambil data toko milik pengguna ini
  const store = await db.prepare("SELECT * FROM stores WHERE user_id = ?").bind(user.id).first()
  if (!store) return c.redirect('/seller/register') // Arahkan ke pendaftaran jika belum punya toko

  // Ambil produk-produk khusus milik toko ini
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="bg-white p-6 rounded-sm shadow-sm border border-gray-200 flex flex-col md:flex-row justify-between items-center">
          <div className="flex items-center space-x-4">
             ...
             <div>
                ...
                <p className="text-sm text-gray-500">Dasbor Penjual • <a href={`/store/${store.slug}`} target="_blank" className="text-black hover:underline">Lihat Toko Publik ↗</a></p>
             </div>
          </div>
          <div className="mt-4 md:mt-0 flex space-x-3">
             <a href="/seller/products/new" className="bg-black text-white px-6 py-2.5 rounded-sm font-bold text-sm uppercase tracking-wide hover:bg-gray-800 transition-colors">
                + Tambah Produk
             </a>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
           <div className="bg-white p-6 border border-gray-200 rounded-sm shadow-sm text-center">
             <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-2">Total Produk</p>
             <h3 className="text-3xl font-black text-gray-900">{products.length}</h3>
           </div>
           <div className="bg-white p-6 border border-gray-200 rounded-sm shadow-sm text-center">
             <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-2">Pengikut (Followers)</p>
             <h3 className="text-3xl font-black text-gray-900">{store.followers_count || 0}</h3>
           </div>
           <div className="bg-white p-6 border border-gray-200 rounded-sm shadow-sm text-center">
             <p className="text-xs text-gray-500 font-bold uppercase tracking-wider mb-2">Rating Toko</p>
             <h3 className="text-3xl font-black text-amber-500">★ {(store.rating as number).toFixed(1)}</h3>
           </div>
        </div>

        {/* Tabel Etalase Produk Saya */}
        <div className="bg-white rounded-sm shadow-sm border border-gray-200 overflow-hidden">
          ...
          {products.length === 0 ? (
             <div className="p-12 text-center">
                <p className="text-gray-500 mb-4">Anda belum menambahkan produk apapun untuk dijual.</p>
                <a href="/seller/products/new" className="text-black font-bold border border-black px-6 py-2 rounded-sm hover:bg-gray-50">Mulai Jualan</a>
             </div>
          ) : (
                 ...
                 <tbody className="text-sm text-gray-700">
                   {products.map((p: any) => (
                     <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50">
                       ...
                       <td className="p-4">
                          <span className={`px-2 py-1 rounded-sm text-[10px] font-bold uppercase ${p.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
